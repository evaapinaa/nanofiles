...
public class NanoFiles {
	...
	public static void main(String[] args) {
		...
		if (args.length > 1) {
			...
			// Establecemos el directorio compartido especificado (si no se ha especificado ninguno, sigue con el default)
			sharedDirname = args[0];
		}

		db = new FileDatabase(sharedDirname); //crea el directorio (si es necesario)

		// Creamos el controlador que aceptará y procesará los comandos
		//NanoFiles crea NFController(), este implementa la logica de la negociacion para establecer la conexión
