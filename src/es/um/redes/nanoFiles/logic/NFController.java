...
public class NFController {
	...
	public NFController() {
		...
		currentState = LOGGED_OUT; //Como se encuentra el peer en el estado actual!!
	...
	public void processCommand() {
		...
		if (!canProcessCommandInCurrentState()) { //no proceso NA jiji 
			return;
		}
		/*
		...
		switch (currentCommand) { //se inicializa con lo que escriba en el shell ¿?
		//si es login se instancian .CON_LOGIN y .COM_NICKNAME con la ip y nickname que se escriba con el login
		case NFCommands.COM_MYFILES:
			showMyLocalFiles(); // Muestra los ficheros en el directorio local compartido
			break;
		case NFCommands.COM_LOGIN:
			if (NanoFiles.testMode) { //si es tru se ejecutará esto
				//y esto que cojones es que tiene hasta excepcion jaja
				//es sencillamente entre comillas:
				try {
					controllerDir.testCommunicationWithDirectory(directory);
					//controllerDIr es un objeto creado con NFContorllerLogicDir para implementar la logica de negocios con el directorio
					//metodo que le pasas la ip del servidor del directorio y crea la comunicacion entre el directorio y el nanofiles¿?
					return;
				} catch (IOException e) {
					e.printStackTrace();
	...
	public boolean canProcessCommandInCurrentState() {
		...
		//currentCommand = comando que se acaba de teclear
		...
		case NFCommands.COM_LOGIN:
			if (currentState != LOGGED_OUT) {
				...
				System.err.println("* You cannot login because you are already logged in to the directory");
			}
			break;
		...
		case NFCommands.COM_LOGOUT:
			if (currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot logout because you are not logged in to the directory");
			}
			
			else if (currentState == SERVER) {
				commandAllowed = false;
				System.err.println("* You cannot logout because you are a server");
			}
		...
		case NFCommands.COM_USERLIST:
			if (currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot get userlist because you are not logged in to the directory");				
			}
		...
		case NFCommands.COM_FILELIST:
			if (currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot get filelist because you are not logged in to the directory");				
			}
			break;
		...
		case NFCommands.COM_BGSERVE:
			if (currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot launch peer as background server because you are not logged in to the directory");				
			}
			else if (currentState == SERVER) {
				commandAllowed = false;
				System.err.println("* You cannot launch peer as background server because you are already a server ");
			}
		...
		case NFCommands.COM_FGSERVE:
			if (currentState == SERVER) {
				commandAllowed = false;
				System.err.println("* You cannot launch peer as foreground serveer because you are already a server");
			}
		...
		case NFCommands.COM_DOWNLOADFROM:
			if (currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot download because you are not logged in to the directory");				
			}
			break;
		...
		case NFCommands.COM_SEARCH:
			if (currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot search files because you are not logged in to the directory");
			}
			break;
		...
		case NFCommands.COM_STOP_SERVER:
			if (currentState != SERVER) {
				commandAllowed = false;
				System.err.println("* You cannot search files because you are not a server");
			}
			break;
		...
		case NFCommands.COM_QUIT:
			if (currentState != LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot quit because you are logged in to the directory");
	...
	private void updateCurrentState(boolean success) {
		//Segun el comando tecleado se actualiza el estado en 
		//el que se encuentra el usuario. Dependiendo del estado
		//en el que se encuentre, podrá ejecutar unos comandos
		//y otros no
		if (!success) {
			return;
		}
