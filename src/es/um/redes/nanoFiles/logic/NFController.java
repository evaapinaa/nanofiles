...
public class NFController {
	 ...
	 */
	private static final byte LOGGED_OUT = 0; // peer estado inicial antes de ejecutar login
	...
	private static final byte LISTENING = 2;
	private static final byte LISTENING_BGSERVE = 3;
	...
	public void processCommand() {
		...
		case NFCommands.COM_LOGIN:
			if (NanoFiles.testMode) {
				try {
					controllerDir.testCommunicationWithDirectory(directory); // directory -> ip servidor
					return;
				} catch (IOException e) {
					e.printStackTrace();
	...
	public boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_LOGOUT:
			...
			break;
		case NFCommands.COM_USERLIST:
			if (currentState != LOGGED_IN && currentState != LISTENING_BGSERVE) {
				commandAllowed = false;
				System.err.println("* You cannot list users because you are not logged in the directory");
			}
			break;
		case NFCommands.COM_PUBLISH:
			if (currentState != LISTENING_BGSERVE) {
				commandAllowed = false;
				System.err.println("* You cannot publish files because you are not listening in the directory");
			}
			break;
		case NFCommands.COM_FGSERVE:
			if (currentState != LOGGED_IN) {
				commandAllowed = false;
				System.err.println(
						"* You cannot foreground serve your files because you are not logged in the directory");
			}
			break;
		case NFCommands.COM_DOWNLOADFROM:
			if (currentState != LOGGED_IN && currentState != LISTENING_BGSERVE) {
		...
		case NFCommands.COM_BGSERVE:
			if (currentState != LOGGED_IN) {
				commandAllowed = false;
				System.err.println(
						"* You cannot background serve your files because you are not logged in the directory");
			}
			break;
		case NFCommands.COM_STOP_SERVER:
			if (currentState != LISTENING_BGSERVE) {
				commandAllowed = false;
				System.err.println("* You cannot stop the server because you are not listening in the directory");
			}
			break;
	...
	private void updateCurrentState(boolean success) {
		...
		case NFCommands.COM_BGSERVE: {
			currentState = LISTENING_BGSERVE;
			break;
		}
