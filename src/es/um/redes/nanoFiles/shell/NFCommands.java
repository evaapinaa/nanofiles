...
public class NFCommands {
	 ...
	 */
	private static final Byte[] _valid_user_commands = { COM_QUIT, COM_LOGIN, COM_USERLIST, COM_FILELIST, COM_MYFILES,
			COM_FGSERVE, COM_BGSERVE, COM_PUBLISH, COM_STOP_SERVER, COM_DOWNLOADFROM, COM_SEARCH, COM_DOWNLOAD,
			COM_LOGOUT, COM_SLEEP, COM_HELP, COM_SOCKET_IN };

	/**
	 * cadena exacta de cada orden
	 */
	private static final String[] _valid_user_commands_str = { "quit", "login", "userlist", "filelist", "myfiles",
			"fgserve", "bgserve", "publish", "stopserver", "downloadfrom", "search", "download", "logout", "sleep",
			"help" };

	/**
	 * Mensaje de ayuda para cada orden
	 */
	private static final String[] _valid_user_commands_help = { "quit the application",
			"log into <directory> using <nickname>", "show list of users logged into the directory",
			"show list of files tracked by the directory", "show contents of local folder (files that may be served)",
			"run file server in foreground (blocking)", "run file server in background (non-blocking)",
			"publish list of served files to the directory", "stop file server running in background",
			...
			"download the file identified by <hash> from all available server(s)", "log out from the current directory",
			"sleep during <num> seconds", "shows this information" };
	...
	public static byte stringToCommand(String comStr) {
		...
		// Si no se corresponde con ninguna cadena entonces devuelve el código de
		// comando no válido
		return COM_INVALID;
	}
	...
	public static void printCommandsHelp() {
		...
		for (int i = 0; i < _valid_user_commands_str.length; i++) {
			System.out.println(
					String.format("%1$15s", _valid_user_commands_str[i]) + " -- " + _valid_user_commands_help[i]);
		}
	}
}
