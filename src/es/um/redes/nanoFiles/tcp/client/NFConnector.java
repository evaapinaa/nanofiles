...
public class NFConnector {
	...
	public boolean downloadFile(String targetFileHashSubstr, File file) throws IOException {
		 ...
		 */
		PeerMessage requestMessage = new PeerMessage(PeerMessageOps.OPCODE_DOWNLOAD,
				(byte) targetFileHashSubstr.length(), targetFileHashSubstr);
		requestMessage.writeMessageToOutputStream(dos);
		/*
		 ...
		 */
		// Escribir los datos del fichero recibido
		if (responseMessage.getOpcode() == PeerMessageOps.OPCODE_DOWNLOAD_RESPONSE) {
			try (FileOutputStream fos = new FileOutputStream(file)) {
				fos.write(responseMessage.getFileData());
				fos.close();
			} catch (IOException e) {
				System.err.println("Error writing file data: " + e.getMessage());
				return false;
			}

			String calculatedHash = FileDigest.computeFileChecksumString(file.getAbsolutePath());
			if (calculatedHash.equals(responseMessage.getFileHash())) {
				downloaded = true;
			} else {
				System.err.println("La verificación del hash del archivo descargado ha fallado.");
				return false;
			}
		} 
		else if (responseMessage.getOpcode() == PeerMessageOps.OPCODE_FILE_NOT_FOUND) {
			System.err.println(PeerMessageOps.opcodeToOperation(PeerMessageOps.OPCODE_FILE_NOT_FOUND));
			return false;
		} 
		else if (responseMessage.getOpcode() == PeerMessageOps.OPCODE_AMBIGUOUS_HASH) {
			System.err.println(PeerMessageOps.opcodeToOperation(PeerMessageOps.OPCODE_AMBIGUOUS_HASH));
			return false;
		}
	...
	public boolean downloadFileChunk(String targetFileHashSubstr, File file) throws IOException {
		boolean result = false;
		PeerMessage requestMessage = new PeerMessage(PeerMessageOps.OPCODE_DOWNLOAD_CHUNK,
				(byte) targetFileHashSubstr.length(), targetFileHashSubstr);
		requestMessage.writeMessageToOutputStream(dos);
		PeerMessage responseMessage = PeerMessage.readMessageFromInputStream(dis);
		if (responseMessage.getOpcode() == PeerMessageOps.OPCODE_DOWNLOAD_CHUNK_RESPONSE) {
